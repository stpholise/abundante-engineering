...
type Service = { key: string; title: string; text: string };

const Page = () => {
  const { t } = useTranslation("common");
  const services = t("sections.services.items", {
    returnObjects: true,
  }) as Service[];
  return (
          ...
          <div className="   grid grid-cols-3 justify-between gap-4 w-full py-6 px-1      dark:border-gray-700">
            {services.map((service) => (
              <div className="" key={service.key}>
                <ServiceCard title={service.title} text={service.text} value={service.key} />
              </div>
            ))}
          </div>
        </div>
